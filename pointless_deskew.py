...
class PointlessDeskewImageProcessor:
    ...
    def determine_skew(
        ...
        if plot_visualization:
            self.visualize_edges(edges)  # Visualize detected edges in the image
            self.visualize_image_and_hough_space(
                img_gray, accumulator, angles, distances
            )  # Visualize the Hough space
            self.visualize_hough_lines(
                img_gray, accumulator, angles, distances, angles_peaks, dists_peaks
    ...
    def orientation_rotation_estimation(
        self, img: Image, predictor: ocr_predictor, tokenizer: BertTokenizer
    ):
        """
        ...
        for temp_file in temp_files:
            ...
            result = predictor([page]).export()["pages"][
                0
        ...
        best_index, scores_normalized = self.analyze_ocr_results(ocr_results, tokenizer)
    ...
    def process_and_display_image_in_streamlit_app(
        ...
        if allow_up_to_180_degrees:
            _, rotated_image, _, _ = self.orientation_rotation_estimation(
                rotated_image, self.predictor, self.tokenizer
            )
